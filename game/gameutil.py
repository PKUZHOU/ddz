...
import copy
import random
from .card_util import ordinalTransfer
#发牌
def game_init(players, playrecords, train):

        #洗牌
        cards = range(54)
        cards = random.sample(cards,54)
        cards = ordinalTransfer(cards)
        #排序
        public_cards = cards[:3]
        p1_cards = cards[3:20]
        p2_cards = cards[20:37]
        p3_cards = cards[37:54]
        players[0].cards_left = playrecords.cards_left1 = p1_cards+public_cards
        players[1].cards_left = playrecords.cards_left2 = p2_cards
        players[2].cards_left = playrecords.cards_left3 = p3_cards
        playrecords.public_cards = public_cards
...
def choose(next_move_types, next_moves, last_move_type, last_move, cards_left, model, RL, agent, game, player_id, action):
    ...
    #训练model
        ...
        if action[3][action[2]] == 34347:
            return "buyao", []
        elif action[3][action[2]] == 34348:
            return "yaobuqi", []
        else:
